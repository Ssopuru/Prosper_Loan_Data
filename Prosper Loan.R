...
#checking the output of the newly created data frame for blank space and n/a
View(loan1)
names(loan1)
n_unique(loan1)
sum(is.na(loan1))
...
#Converting column "incomerange" into ordered(factor)categorical types
loan1$incomerange <- factor(loan1$incomerange, ordered = TRUE)

str(loan1)

#using a bar-chart to show the demography of Borrower's
ggplot(loan1, aes(x = borrowerstate)) + 
  geom_bar() +
...
ggplot(loan1, aes(x = month, fill="red")) + 
  geom_bar() +
  labs(title = "Month Distribution",
       x = "Month")
#*******month with the highest number of borrowers is 1 == January*

#What year has the highest number of borrowers?
ggplot(loan1, aes(x = year, fill="red")) + 
  geom_bar() +
...
#what is the duration of the loan(A density-chart showing the distribution of Term)
...
ggarrange(
  ggplot(data = loan1, mapping = aes(x = incomerange, fill = isborrowerhomeowner)) +
    geom_bar() +
    labs(title = "correlation Btw incomerange & isborrowerhomeowner", x = "Incomerange", y = "isborrowerhomeowner") +
    theme(axis.text.x = element_text(angle = 90),
          panel.background = element_blank(), plot.title = element_text(size = 14)),
  ggplot(data = loan1, mapping = aes(x = employmentstatus, fill = isborrowerhomeowner)) +
    geom_bar() +
    labs(title = "EmploymentStatus & IsBorrowerHomeowner", x = "employmentstatus", y = "isborrowerhomeowner") +
    theme(axis.text.x = element_text(angle = 90),
          panel.background = element_blank(), plot.title = element_text(size = 14)))
...
ggplot(loan1, aes(x = statedmonthlyincome, y = frequency(statedmonthlyincome))) +
  geom_bar(stat = "identity")

qplot(seq_along(loan1$statedmonthlyincome), loan1$statedmonthlyincome)

ggplot(loan1, aes(x = monthlyloanpayment, color = "red")) + geom_bar()

ggplot(loan1, aes(x = statedmonthlyincome)) + geom_histogram(bins = 20)

ggplot(loan1, aes(x = statedmonthlyincome)) + geom_bar()
